...
def comp_tracer_total_mass_2d(var, tracer_name):
    ...
    if var.initial_value is None:
        ...
        # alter the initial value to record tracer transitioning through source and sink term
        # and boundary terms
        var.update_value += var.solver_obj.options.timestep * boundary_terms

        if var.solver_obj.options.tracer_source_2d is not None and var.solver_obj.options.tracer_sink_2d is not None:
            var.update_value += var.solver_obj.options.timestep * \
                assemble((var.solver_obj.options.tracer_source_2d-(scalar_func*var.solver_obj.options.tracer_sink_2d))*H*dx)
        elif var.solver_obj.options.tracer_source_2d is not None and var.solver_obj.options.tracer_sink_2d is None:
            var.update_value += var.solver_obj.options.timestep * assemble(var.solver_obj.options.tracer_source_2d*H*dx)
        elif var.solver_obj.options.tracer_source_2d is None and var.solver_obj.options.tracer_sink_2d is not None:
            var.update_value += var.solver_obj.options.timestep * assemble(-scalar_func*var.solver_obj.options.tracer_sink_2d*H*dx)
...
def comp_tracer_total_mass_2d_cons(var, tracer_name):
    ...
    equation accounting for tracer leaving through boundary conditions and source and sink terms
    :arg var: class:`DiagnosticCallback`; callback object used to input values and
                                          record output values
    :arg tracer_name :class:`string` of function name of interest
    """
    ...
    if var.initial_value is None:
        ...
        if var.solver_obj.options.tracer_source_2d is not None and var.solver_obj.options.tracer_sink_2d is not None:
            ...
            var.update_value += var.solver_obj.options.timestep * assemble(-scalar_func*var.solver_obj.options.tracer_sink_2d*H*dx)
        elif var.solver_obj.options.tracer_depth_integ_source is not None and var.solver_obj.options.tracer_depth_integ_sink is not None:
            var.update_value += var.solver_obj.options.timestep * \
                assemble((var.solver_obj.options.tracer_depth_integ_source-(scalar_func*var.solver_obj.options.tracer_depth_integ_sink))*dx)
        elif var.solver_obj.options.tracer_depth_integ_source is not None and var.solver_obj.options.tracer_depth_integ_sink is None:
            var.update_value += var.solver_obj.options.timestep * assemble(var.solver_obj.options.tracer_depth_integ_source*dx)
        elif var.solver_obj.options.tracer_depth_integ_source is None and var.solver_obj.options.tracer_depth_integ_sink is not None:
            var.update_value += var.solver_obj.options.timestep * assemble(-scalar_func*var.solver_obj.options.tracer_depth_integ_sink*dx)
