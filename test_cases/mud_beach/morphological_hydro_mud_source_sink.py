...
def hydrodynamics_only(boundary_conditions_fn, mesh2d, bathymetry_2d, uv_init,\
    ...
    cfactor = th.Function(P1_2d).interpolate(th.Constant(0.002))
...
def morphological(boundary_conditions_fn, morfac, morfac_transport, suspendedload, convectivevel, bedload, angle_correction, slope_eff, seccurrent, sediment_slide, \
        ...
        if morfac_transport == True:
            ...
            if double_factor == True:
                ...
                if round(t_new, 2)%t_export == 0:
                    ...
                    if round(t_new, 2)%(18*t_export) == 0:
                        xaxisthetis1 = []
                        baththetis1 = []
                        ...
                        df.to_csv(str(t_new) + "_100_2.csv", index = False)                    
    ...
    """
    settling_velocity = th.Constant(0.0005)
    ...
    cfactor = th.Function(P1_2d).interpolate(th.Constant(0.002))#th.conditional(depth > ksp, 2*((2.5*th.ln(11.036*hclip/ksp))**(-2)), th.Constant(0.0)))
    # mu - ratio between skin friction and normal friction
