...
def wet_dry_interface_monitor(mesh, alpha=30.0, beta=1.0):  
    """
    ...
    uv_dx_new2 = project(conditional(uv_dx_new > Constant(0.0), uv_dx_new, Constant(0.0)), P1)
    mon_init = project(sqrt(1.0 + alpha * uv_dx_new2), P1)
    ...
    """
    K2 = 20*(0.4**2)/4
    b = (inner(tau, H2)*dx)+(K2*inner(grad(tau), grad(H2))*dx) - (K2*(tau*inner(grad(H2), n)))*ds
    b -= inner(tau, H)*dx
    solve(b == 0, H2)    
    """
    return H
