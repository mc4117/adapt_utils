...
df_real = pd.read_csv('fixed_output/bed_trench_output_uni_2.0.csv')
df_real1 = pd.read_csv('fixed_output/bed_trench_output_uni_0.8.csv')
...
plt.loglog([0.25, 0.5, 1, 2], error_list, '-o')
plt.ylabel('Error norm (m)')
plt.xlabel(r'$\Delta x$ (m)')
plt.show()


logx = np.log([0.25, 0.5, 1, 2])
...
plt.loglog([0.1, 0.25, 0.5, 1, 2], error_listexp, '-o')
plt.ylabel('Error norm (m)')
plt.xlabel(r'$\Delta x$ (m)')
plt.show()
